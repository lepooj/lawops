            ...
            >
              <div>
                <p className="text-sm font-medium text-indigo-300">
                  Explore a demo analysis
                </p>
                <p className="text-xs text-indigo-400/60">
                  {demoMatter.title} — see structured output, authorities, and
                  verification
                </p>
              </div>
            ...
            <tbody>
              {matters.map((matter) => (
                ...
                >
                  ...
                  <td className="py-3 pr-4">
                    <Badge variant={STATUS_BADGE_VARIANT[matter.status]}>
                      {matter.status}
                    </Badge>
                  </td>
      ...
      <div className="mt-16 flex flex-col items-center text-center">
        <div className="rounded-lg border border-zinc-800/60 bg-zinc-900/40 px-8 py-10">
          <p className="text-sm text-zinc-400">
            No {activeFilter.toLowerCase()} matters.
          </p>
        </div>
      </div>
    );
    ...
    <div className="mt-16 flex flex-col items-center text-center">
      <div className="max-w-md rounded-lg border border-zinc-800/60 bg-zinc-900/40 px-8 py-12">
        ...
        <p className="mt-3 text-sm leading-relaxed text-zinc-400">
          Create a matter, enter structured legal context, and generate an
          AI-assisted analysis with cited authorities, risk assessment, and
        ...
        <div className="mt-6 space-y-3 text-left">
          {[
            ...
            { step: "2", label: "Complete intake", desc: "Jurisdiction, facts, objective, and known authorities" },
            { step: "3", label: "Upload documents", desc: "PDF, DOCX, TXT, or document photos" },
            { step: "4", label: "Run analysis", desc: "Structured AI analysis with verification labels" },
          ].map(({ step, label, desc }) => (
